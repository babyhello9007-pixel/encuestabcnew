...
const canonicalProvince = (raw: string) => {
  const n = normalizeName(raw);
  return PROV_ALIASES[n] || Object.keys(SEATS).find((p) => normalizeName(p) === n) || raw;
};
...
const BASE_DISTRIBUTION: Record<string, number> = { PP: 0.34, PSOE: 0.31, VOX: 0.16, SUMAR: 0.13, ERC: 0.03, JUNTS: 0.03 };
const buildSimData = (seatsMap: Record<string, number>): Record<string, ProvinceModel> => Object.fromEntries(Object.entries(seatsMap).map(([prov, seats]) => {
  ...
  Object.entries(BASE_DISTRIBUTION).forEach(([p, w]) => (votes[p] = Math.floor(validVotes * w)));
  votes.OTROS = Math.floor(validVotes * 0.03);
  return [prov, { census: Math.floor(validVotes * 1.35), validVotes, p: votes } satisfies ProvinceModel];
}));
...
const MapView = memo(function MapView({ winners, onSelect, selected }: { winners: Record<string, string>; onSelect: (p: string) => void; selected: string }) {
  ...
  useEffect(() => {
    ...
    const map = L.map(mapRef.current).setView([40.2, -3.7], 5.8);
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", { attribution: "&copy; OpenStreetMap contributors" }).addTo(map);
    leafletRef.current = map;
    fetch("/data/georef-spain-provincia.geojson")
      .then((r) => r.json())
      .then((geojson) => {
        const layer = L.geoJSON(geojson, {
          style: (feature: any) => {
            const nRaw = feature?.properties?.provincia || feature?.properties?.name || "";
            const n = canonicalProvince(nRaw);
            const c = winners[n] || "#64748b";
            return { color: "#111827", weight: selected === n ? 2 : 1, fillColor: c, fillOpacity: 0.55 };
          },
          onEachFeature: (feature: any, layer) => {
            const nRaw = feature?.properties?.provincia || feature?.properties?.name || "Provincia";
            const n = canonicalProvince(nRaw);
            layer.bindTooltip(n);
            layer.on("click", () => onSelect(n));
          },
        }).addTo(map);
        geoLayerRef.current = layer;
      });
    return () => { map.remove(); leafletRef.current = null; };
  }, [onSelect, selected, winners]);
  ...
  return <div><div ref={mapRef} className="h-96 w-full rounded" /><div className="mt-1 text-[11px] text-slate-400">Leaflet | © OpenStreetMap contributors</div></div>;
});
...
export default function SimuladorBCGuide() {
  ...
  const selectedData = simData[selectedProv] || Object.values(simData)[0];
  ...
  return <div className={dark ? "min-h-screen text-slate-100" : "min-h-screen text-slate-900"} style={{ background: dark ? "radial-gradient(circle at 20% 10%, #1e293b 0%, #020617 50%, #02030a 100%)" : "radial-gradient(circle at 20% 10%, #ffffff 0%, #e2e8f0 55%, #cbd5e1 100%)" }}>
    <div className="mx-auto max-w-7xl px-4 py-6">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2"><img src="/favicon.png" alt="BC" className="h-7 w-7" /><h1 className="text-2xl font-extrabold">Simulador BC · Elecciones Generales y autonómicas</h1></div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border border-white/20 bg-white/10 p-3 backdrop-blur-xl shadow-[0_10px_40px_rgba(0,0,0,0.25)] ">
          ...
          <div className="grid gap-2 md:grid-cols-2">
            {Object.entries(selectedData.p).map(([p, v]) => <label key={p} className="flex items-center justify-between rounded bg-black/20 p-2 text-xs"><span>{p}</span><input type="number" value={v} onChange={(e) => setSimData((prev) => ({ ...prev, [selectedProv]: { ...(prev[selectedProv] || selectedData), p: { ...(prev[selectedProv]?.p || selectedData.p), [p]: Math.max(0, Number(e.target.value || 0)) } } }))} className="w-28 rounded bg-transparent text-right"/></label>)}
          </div>

              ...
              <div className="space-y-5">
                {partyLeaders.length > 0 && (
                  <div className="glass-surface p-4 rounded-2xl">
                    <h3 className="text-lg font-bold text-slate-900 mb-3">Líderes de Partidos (configurados en Admin)</h3>
                    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                      {partyLeaders.map((leader) => {
                        const partyKey = resolvePartyKey(leader.partyKey, generalPartyMetaLookup);
                        const partyColor = generalPartyMetaLookup[partyKey]?.color || "#64748b";
                        const partyName = generalPartyMetaLookup[partyKey]?.name || leader.partyKey;
                        return (
                          <div key={`${leader.partyKey}-${leader.leaderName}`} className="rounded-xl border p-3 bg-white" style={{ borderColor: `${partyColor}66` }}>
                            <div className="flex items-center gap-3">
                              <PartyLogo src={leader.photoUrl} alt={leader.leaderName} partyName={leader.leaderName} size={44} />
                              <div>

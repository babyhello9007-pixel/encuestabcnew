...
interface NocheElectoralRow {
  ...
  results: { party_id: number; party_key: string; display_name: string; color: string; porcentaje_voto: number; escanos: number | null; is_projection: boolean; is_final: boolean; }[];
}
      ...
      try {
        ...
        try {
          const { data } = await supabase
            ...
            .select("id,election_date,region_name,region_flag_url,close_at,electiondirect_results(porcentaje_voto,escanos,is_projection,is_final,party_configuration(id,party_key,display_name,color))")
                        ...
                        <div style={{ display: "grid", gap: 4 }}>
                          {r.results?.map((pr, idx) => (
                            <div key={`${pr.party_id}-${idx}`} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12 }}>
                              ...
                              <b>{pr.porcentaje_voto.toFixed(2)}%</b>
